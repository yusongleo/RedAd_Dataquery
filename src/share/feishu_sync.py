...
class FeishuSync:
    ...
    def _create_table_and_update_config(self, app_token: str, advertiser_id: str, advertiser_name: str) -> Optional[str]:
        """创建新表"""
        token = self._get_token()
        if not token: return None

        clean_name = "".join(c for c in advertiser_name if c.isalnum())
        table_name = f"{clean_name}_{int(time.time())}"[-50:] 
    ...
    def _check_duplicate(self, app_token: str, table_id: str, acc_name: str, start_ts: int, end_ts: int) -> bool:
        ...
        try:
            ...
            # 使用 filter 参数筛选账户名称 (注意：需要 URL 编码或依赖 requests 自动处理)
            # 语法：CurrentValue.[字段名] = "值"
            filter_str = f'CurrentValue.[账户名称] = "{acc_name}"'
            
            params = {
                "filter": filter_str,
                "page_size": 100 # 检查最近 100 条同名账户记录足够了
            }
            ...
            if res.get("code") == 0 and res.get("data") and res.get("data").get("items"):
                ...
                for item in items:
                    ...
                    # 比对日期时间戳 (飞书返回的日期类型也是时间戳)
                    if fields.get("开始日期") == start_ts and fields.get("结束日期") == end_ts:
                        return True
            return False
            
        except Exception as e:
            # 查重出错不应阻断写入，默认返回 False (允许写入，宁可重不可缺)
            # print(f"⚠️ 查重跳过: {e}") 
            return False

    def sync_to_feishu(self, metrics: Dict, advertiser_id: str, advertiser_name: str, start_date: str, end_date: str, retry_count=0):
        """核心同步逻辑"""
        # 1. 路由配置
        ...
        if target and target.get("table_id") and retry_count == 0:
            ...
            if not default_app_token:
                print("❌ 缺少 default_app_token，无法创建表格")
                return
            new_id = self._create_table_and_update_config(default_app_token, advertiser_id, advertiser_name)
            if new_id:
                target_conf = {"app_token": default_app_token, "table_id": new_id}
            else:
                return
        ...
        # 2. [新增] 执行查重 (仅在非重试且非新建表的情况下检查)
        # 如果是刚新建的表(retry_count>0)，肯定没有数据，不用查
        if retry_count == 0:
            print(f"🔍 正在检查 [{advertiser_name}] 的历史记录...")
            is_dup = self._check_duplicate(target_conf['app_token'], target_conf['table_id'], advertiser_name, ts_start, ts_end)
            if is_dup:
            ...
            if res_json.get("code") == 0:
                ...
                if any(x in msg for x in error_triggers):
                    if retry_count < 1:
                        ...
                        self.sync_to_feishu(metrics, advertiser_id, advertiser_name, start_date, end_date, retry_count=1)
                    else:
